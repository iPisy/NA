...
int main(){
    ...
    for(int i=0;i<3;i++){
        ...
        //Generate 2 more points and erase the points at the edge to avoid enormous derivative value. 
        spline.setControlPointsList_List(BezierSpline::connectInitList(
            vector<CurvePointList>{upper.generatePointList(bar[i]+2,1,0),lower.generatePointList(bar[i]+2,1,1)}));
