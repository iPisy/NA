...
\begin{description}
  ...
  which means it can be calculated faster. 
  And whole procedure of generating spline is totally different from generating PpFormSpline, given parameter of $n=3$. \\
  Thus, I derived this class from spline and implement it by using lemmas in book.\\
  It allows 5 kind of boundary conditions (all defined in book\cite{Defn_3_5}), done by override \texttt{virtual void addBoundaryCondition(...);}.\\
...
\begin{description}
  ...
  \begin{description}
    ...
    The second parameter \texttt{FunctionPointList fList} is the interpolation points, they are points on the function. If you defines a function (don't forget to assign the definition domain), you can use its member function 
    \texttt{generatePointList(int number,int derivative\_order=0);} to easily get that.\\
    The third parameter \texttt{const BoundaryCondition\& boundaryCondition=BoundaryCondition\_Periodic\{\}} is the boundary condition. You can objectify a sort of boundary condition class (the sub-class, and offer values it need) and pass it in.\\
    The fourth parameter \texttt{const IndependentVariableList\& knotList=\{\}} is for spline in Thm 3.58, whose knots are not interpolation points. Then you should assign the knots by this parameter.\\
  ...
  \end{description}
  \item[Specific Spline] To complete a spline interpolation, you just need to construct the spline you want to use, and then call functions mentioned above.
\end{description}
